## QUESTION
When you have to design a standard cell library, you need to choose a number of combinatorial cells.
...
**orbits** computes the orbits in the natural action of the symmetric group $S_n$ on the set of n-input Boolean functions $X_n=B^{B^n}$.
...
**powerset** computes the orbits generated by the natural action of the symmetric group $S_n$ on the set of n-input Boolean functions $X_n=B^{B^n}$ using the power set method.
...
It gives a standard cell called a **buffer**.
...
3. The length of the integer strings used to represent the signatures is limited to 8 192 characters.
